...
{
public:
	__device__ camera( point3 lookfrom,//起点
			point3 lookat,//终点
			vec3 Vup,//向上的向量
			float vFOV, //垂直的FOV
		...
		lower_left_corner = origin - horizontal / 2.0f - vectical / 2.0f - focus_dist * w;//左下角 视窗起点

		lens_radius = aperture / 2.0f;
	 }
	__device__ ray get_ray(float s,float t, curandState* local_rand_state)//从相机中心生成射线
	{
		//Defocus Blur 失焦模糊（景深）
		vec3 rd = lens_radius * random_in_uint_disk(local_rand_state);//在透镜内产生随机点
